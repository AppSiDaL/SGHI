...
export default function index (): JSX.Element {
  ...
  useEffect(() => {
    ordenesService
      .getItems()
      .then((data: any) => {
        setItems(data.data as Orden[])
      })
      .catch((err) => { console.log(err) })
  }, [])
  const getSeverity = (product: Orden): 'success' | 'warning' | 'danger' | null => {
  ...
  return (
    <>
      <DataTableComponent
        ...
        items={items}
        setItems={setItems as React.Dispatch<SetStateAction<Orden[] | Herramienta[] | Part[] | Movimiento[]>>}
        columns={columns}
        item={item}
        setItem={setItem as React.Dispatch<SetStateAction<Orden | Herramienta | Part | Movimiento>>}
      />
    </>
  )
}
